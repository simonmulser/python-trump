...
with open('tweets.csv', 'w') as file:
	...
	writer.writerow(['created_at', 'text', 'text_truncated', 'source', 'retweet_count', 'favorite_count', 'link'])
...
with open('tweets.csv', 'a') as file:
	...
	while current > oldest:
		...
		for tweet in tweets:
			writer.writerow([tweet.created_at, tweet.text, tweet.truncated, tweet.source, tweet.retweet_count, tweet.favorite_count, 'https://twitter.com/realdonaldtrump/status/{}'.format(tweet.id)])
			current = tweet.id